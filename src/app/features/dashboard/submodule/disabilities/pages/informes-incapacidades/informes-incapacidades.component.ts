...
/**
 * Fecha del backend ('yyyy-MM-dd' o ISO) como Date local, para que la tabla
 * estandar ordene por fecha: un 'yyyy-MM-dd' con `new Date()` se leeria en UTC
 * y en Colombia caeria el dia anterior.
 */
export function aFecha(v: string | null | undefined): Date | null {
  if (!v) return null;
...
/** Texto de la fecha con el patron del `date` pipe de antes; '' si no hay fecha. */
export function fechaTexto(v: string | null | undefined, patron = 'dd/MM/yyyy'): string {
  const d = aFecha(v);
  return d ? formatDate(d, patron, 'en-US') : '';
}

/** Tono de chip de la tabla estandar equivalente a las clases `ges-chip-*` del modulo. */
...
/** Fila del top con su puesto fijo (no cambia al reordenar la tabla). */
export type FilaTopConPuesto = FilaTopPersona & { puesto: number };
...
export class InformesIncapacidadesComponent implements OnInit {
  ...
  // ── Tablas (estandar: busqueda, filtros, orden y copiado los da la tabla) ──

  readonly columnasRecurrencia: ColumnaTabla<FilaRecurrencia>[] = [
    ...
    { id: 'empresa', header: 'Empresa / finca', valor: (f) => f.empresa ?? '', formato: (f) => f.empresa || '—', tarjeta: 'cuerpo' },
    { id: 'oficina', header: 'Oficina', valor: (f) => f.oficina ?? '', formato: (f) => f.oficina || '—', prioridad: 2, tarjeta: 'meta' },
    { id: 'eps', header: 'EPS', valor: (f) => f.eps ?? '', formato: (f) => f.eps || '—', prioridad: 2, tarjeta: 'meta' },
    { id: 'mes', header: 'Mes', valor: (f) => f.mesEtiqueta, tarjeta: 'meta' },
    { id: 'eventos', header: 'Eventos', valor: (f) => f.eventos, align: 'right', tarjeta: 'badge',
      badge: (f) => ({ texto: String(f.eventos), tono: f.eventos >= 4 ? 'danger' : 'warn' }) },
    { id: 'dias', header: 'Dias', valor: (f) => f.dias, align: 'right', tarjeta: 'meta' },
    { id: 'diagnosticos', header: 'Diagnosticos', valor: (f) => f.diagnosticos.join(', '), prioridad: 3, tarjeta: 'cuerpo' },
    { id: 'ips', header: 'IPS', valor: (f) => f.ips.join(', '), prioridad: 3, tarjeta: 'cuerpo' },
    { id: 'fechas', header: 'Fechas', valor: (f) => f.fechas.join(' · '), prioridad: 3, tarjeta: 'cuerpo', minAncho: '220px' },
  ];

  readonly columnasTop: ColumnaTabla<FilaTopConPuesto>[] = [
    { id: 'puesto', header: 'Puesto', valor: (f) => f.puesto, align: 'right', tarjeta: 'meta', ancho: '70px' },
    { id: 'cedula', header: 'Cedula', valor: (f) => f.cedula, tarjeta: 'subtitulo' },
    { id: 'nombre', header: 'Nombre', valor: (f) => f.nombreCompleto, tarjeta: 'titulo', minAncho: '180px' },
    { id: 'empresa', header: 'Empresa / finca', valor: (f) => f.empresa ?? '', formato: (f) => f.empresa || '—', tarjeta: 'cuerpo' },
    { id: 'oficina', header: 'Oficina', valor: (f) => f.oficina ?? '', formato: (f) => f.oficina || '—', prioridad: 2, tarjeta: 'meta' },
    { id: 'eventos', header: 'Incapacidades', valor: (f) => f.eventos, align: 'right', tarjeta: 'meta' },
    { id: 'dias', header: 'Dias', valor: (f) => f.dias, align: 'right', tarjeta: 'meta' },
  ];

  /** El top con su puesto original: al ordenar por otra columna el puesto no se pierde. */
  readonly filasTop = computed<FilaTopConPuesto[]>(() =>
    (this.top()?.filas ?? []).map((f, i) => ({ ...f, puesto: i + 1 })),
  );

  readonly idRecurrencia = (f: FilaRecurrencia) => `${f.cedula}-${f.anio}-${f.mes}`;
  readonly idTop = (f: FilaTopConPuesto) => f.cedula;
