...
@Component({
  ...
  imports: [
    ...
    MatInputModule, MatProgressSpinnerModule, MatSelectModule, MatTooltipModule, ...TABLA_ESTANDAR,
  ],
...
export class SstIncapacidadesComponent implements OnInit {
  ...
  /** La tabla guarda su pagina: al cambiar un filtro de negocio se vuelve a la 1. */
  ...
  /**
   * Columnas de la tabla estandar. La paginacion y la busqueda van al backend
   * (modo servidor) y el endpoint no ordena: por eso ninguna es ordenable.
   */
  readonly columnas: ColumnaTabla<FilaSst>[] = [
    { id: 'codigo', header: 'Codigo', ordenable: false, tarjeta: 'meta',
    ...
    { id: 'finca', header: 'Finca / centro de costo', ordenable: false, tarjeta: 'cuerpo',
      valor: (f) => f.empresa ?? '', formato: (f) => f.empresa || '—' },
    { id: 'tipo', header: 'Tipo', ordenable: false, tarjeta: 'badge',
      valor: (f) => f.tipoIncapacidadEtiqueta ?? '' },
    // Sin orden (modo servidor): el valor es el mismo texto que se ve, y asi se copia.
    { id: 'periodo', header: 'Periodo', ordenable: false, tarjeta: 'subtitulo',
      valor: (f) => `${fechaTexto(f.fechaInicio)} → ${fechaTexto(f.fechaFin)}` },
    { id: 'diagnostico', header: 'Diagnostico', ordenable: false, prioridad: 2, tarjeta: 'cuerpo',
      valor: (f) => f.codigoDiagnostico ?? '', formato: (f) => f.codigoDiagnostico || '—' },
    { id: 'furat', header: 'FURAT', ordenable: false, align: 'center', prioridad: 2, tarjeta: 'meta',
      valor: (f) => f.tieneFurat },
    { id: 'plazo', header: 'Plazo', ordenable: false, prioridad: 2, tarjeta: 'meta',
      valor: (f) => fechaTexto(f.fechaLimite) },
    { id: 'investigacion', header: 'Investigacion', ordenable: false, tarjeta: 'badge',
      valor: (f) => f.investigacion.estadoEtiqueta },
  ];

  readonly idFila = (f: FilaSst) => f.incapacidadId;
  /** Plazo de la investigacion vencido: la fila se resalta. */
  readonly claseFila = (f: FilaSst) => (f.vencida ? 'te-fila--peligro' : '');
  ...
  /** Busqueda libre de la tabla estandar (ya llega con debounce): va al backend desde la pagina 1. */
  buscar(texto: string): void {
    this.filtros.q = texto;
    this.page.set(0); // la tabla ya volvio a su pagina 1
    this.cargar();
  }
